...
Bragg condition: $\sin\theta_B = \lambda/2\Lambda$ with $\Lambda = v_s/f_{RF}$ the acoustic wavelength. Order $m$ exits at frequency $\omega + m\,\omega_{RF}$ — the $+1$ order absorbed a phonon, the $-1$ emitted one; sign is set by geometry (which side the acoustic wave comes from).
...
$$
TeO₂ at $v_s \approx 4.2$ mm/μs: a 1 mm beam gives ~240 ns; focus to 100 μm for ~24 ns. Focus tighter for speed — but too tight and the beam's angular spread exceeds the Bragg acceptance and efficiency dies. Speed vs efficiency is *the* AOM tradeoff.
